...
class WinApp(QWidget):
    ...
    def initUI(self):
        ...
        pixmap = QPixmap('./imges/mong.jpg').scaledToWidth(800)
        lblImage = QLabel(self)
        lblImage.setPixmap(pixmap) 

        lblSize = QLabel(self)
        lblSize.setFont(QFont('NanumGodicCoding', 20))  # 폰트와 폰트사이즈
        lblSize.setStyleSheet('Color: #3F0099;')
        lblSize.setText(f'{pixmap.width()} X {pixmap.height()}')  # mongmong.jpg의 width  x height
        ...
        self.setWindowIcon(QIcon('./imges/iot.png'))
        self.setWindowTitle('이미지 뷰어')
        rect =QRect(300, 300, 300, 300) #x, y, w, h
        self.setGeometry(rect) #같은 이름의 함수를 여러개 선언해놓고 입맛에 맞게 골라쓰리 BR(오버로딩)
        # self.setGeometry(300, 300, 300, 300)
        self.show()    # showFullScreen() 모니터를 꼭 채워서 출력
        self.setCenter()
